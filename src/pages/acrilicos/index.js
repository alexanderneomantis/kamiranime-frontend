...
export default function Acrilicos() {
  const [drawer, setDrawer] = useState(false);
  const {data, loading, search} = useGetProductsByCategory('acrilicos');
...
`
  ...
  return (
    <RootStyle title='Acrilicos | Kamiranime'>
      <BreadCrumb/>
      <Container>
        <Grid container>
          <Grid item xs={12} md={3}>
            <FiltersDesktop category='acrilicos' setFilters={setFilters} filters={filters} range={range} setRange={setRange}/>
